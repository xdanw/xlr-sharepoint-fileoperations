...
#    def __init__(self, url, username, password):
    def __init__(self, tokenServer, url, username, password):
        ...
        self.cookieParams = {'url':'https://' + urlparse(url).hostname, 'username':username, 'password':password}
        self.url = url
        ...
        # print("Exiting SamlTokenProvider.__init__() \r\n")
    ...
    def process_service_token_response(self, response):
        ...
        # print("   process_service_token_response() \r\n")
    ...
    def acquire_authentication_cookie(self, options):
        ...
        # print("... acquire_authentication_cookie() \r\n")

        # It won't be applied to the cookieRequest
        ...
        # logger.info("url: " + url) # This never gets used!
        ### logger.info("self.login: " + self.login)
        ### logger.info("self.cookieParams... " + str(self.cookieParams))


        response = self.cookieRequest.post(self.login, self.token, contentType='application/x-www-form-urlencoded')
    ...
    def acquire_authentication_cookie_original(self, options):
        """Retrieve SPO auth cookie"""
        ### logger = self.logger(self.acquire_authentication_cookie.__name__)

        url = options['endpoint']

        # print "Getting Authentication Cookie..."
        # print "URL: " + url
        session = requests.session()
        ### logger.debug_secrets("session: %s\nsession.post(%s, data=%s)", session, url, self.token)
        session.post(url, data=self.token, headers={'Content-Type': 'application/x-www-form-urlencoded'}, verify="ca.pem")
        ### logger.debug_secrets("session.cookies: %s", session.cookies)
        cookies = requests.utils.dict_from_cookiejar(session.cookies)
        ### logger.debug_secrets("cookies: %s", cookies)
